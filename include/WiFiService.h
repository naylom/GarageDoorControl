...
public:
	...
	WiFiService ();
	~WiFiService ();
	void Begin ( const char* apSSID, const char* apPassword, MNRGBLEDBaseLib* pLED = nullptr );
	void BeginWithConfig ( const char* HostName,
	                       const char* WiFissid,
	                       const char* WiFipwd,
	                       MNRGBLEDBaseLib* pLED = nullptr );
	const char* GetHostName () const;
	IPAddress GetMulticastAddress () const;
	Status GetState () const;
	static String ToIPString ( const IPAddress& address );
	unsigned long GetTime () const;
	float GetAltitudeCompensation () const;
...
protected:
	...
	bool m_useOnboarding = false;    // whether to use onboarding mode
...
private:
	...
	const char* m_apSSID = nullptr;
	const char* m_apPassword = nullptr;
...
public:
	...
	UDPWiFiService ();
	bool Begin ( UDPWiFiServiceCallback pHandleReqData,
	             const char* apSSID,
