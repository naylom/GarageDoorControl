...
DoorState is a class that handles a garage door and uses the StateTable library to maintain the door state and execute
actions as events occur. External events come from a transitory switch which is used to open / close and stop the door.

Further external events comne from a hormann UAP 1 controller that is used to get changes in door information (is closed
/ is open / light on)

The hormann UAP 1 is also used to control the door with commands to start opening, start closing, stop the door and to
turn on the light. These are sent as 1 second pulses. The timing function relies on the MNTimerLib library.
...
constexpr uint32_t SWITCH_DEBOUNCE_MS =
    100;  // min ms between consecutive pin interrupts before signal accepted from manual switch
constexpr uint32_t MAX_SWITCH_MATCH_TIMER_MS =
    2000;                                      // max time pin should be in matched state to be considered a real signal
 ...
 *
 * This constructor initializes a `DoorState` object with the specified pin configurations for controlling and
 * monitoring the door. It sets up the control pins for opening, closing, stopping, and lighting the door, as well as
 * the status pins for detecting the door's state. The constructor also initializes the debounce and match timers for
 * the status pins and sets the initial state of the control pins to off.
 *
 ...
 */
DoorState::DoorState ( pin_size_t OpenPin,
                       pin_size_t ClosePin,
                       pin_size_t StopPin,
                       pin_size_t LightPin,
...
void DoorState::SwitchPressed ( Event )
{
	...
	switch ( GetDoorState() )
	{
		...
		case State::Closing:
			...
			// do not change direction as we need to know what it was doing before we stopped so we can reverse if
			// needed
...
void DoorState::DoEvent ( DoorState::Event eEvent )
{
	...
	( this->*DoorState::StateTableFn [ (uint8_t)GetDoorState() ][ (uint8_t)eEvent ] ) (
	    eEvent );  // Call the appropriate member function based on the current state and event
}
/**
 ...
 *
 * This function collects the states of the light, door open, and door closed pins, both their matched and current
 * states. It then formats these states into a readable string.
 *
...
/// <summary>
//// This function removes the callback for turning off control pins and sets all control pins (open, close, stop,
 ...
 *
 * This constructor initializes a `DoorStatusPin` object with the specified parameters, including the door state,
 * events, pin, debounce time, maximum matched time, match status, pin mode, and initial status. It also sets the pin
 * drive strength - now unneeded.
 *
 ...
 */
DoorStatusPin::DoorStatusPin ( DoorState* pDoor,
                               DoorState::Event matchEvent,
                               DoorState::Event unmatchEvent,
                               pin_size_t pin,
                               uint32_t debouncems,
...
{
	// Set pin to source 7mA and sink 10mA rather than default 2 / 2.5 mA, see
	// https://forum.arduino.cc/t/are-the-zeros-pins-set-to-strong-drive-strength-by-default/404930
